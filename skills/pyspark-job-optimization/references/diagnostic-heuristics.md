...
- 如果同一条业务链在不同日期执行时都会重新解析同一批历史数据，要把它当成重复执行问题处理，优先考虑增量化、按天沉淀中间表，或者把解析动作前移并复用结果。
- 如果某个环节读取的数据超过 10 亿行，或者输入规模超过 2T，要先做量级合理性检查；重点确认上游表规模、分区裁剪、字段裁剪、是否存在重复扫描，以及是否应该提前落中间表。
- 如果单个 stage 读取超过 10 亿行或 2T，或者运行时间超过 30 分钟，且这类超大 stage 超过 4 个，要优先考虑把部分 stage 落表 / 中间表化，避免任务中途失败后前面已经成功的 stage 数据无法复用。
- 如果发现冗余中间列，尤其是排序字段、`rank` / `row_number` / `dense_rank` 这类结果下游没有消费的列，要优先删掉这段计算，而不是只在末端 drop 字段。
- 如果 scan 的是 130 天、360 天这类长历史分区全量表，或者同一批历史数据会被多次重复处理，要优先考虑中间表、增量计算或解析结果复用，避免大数据量重复 scan。
...
- 如果窗口排序、groupBy、join 这类 shuffle 慢点出现时，先分清是总数据量过大还是数据倾斜更严重；不要默认都归为总量问题。
- 先给低风险优化，再给结构性改造。
...
- 对于上游出现、下游没有消费、最终也没有写入结果表的字段，优先按冗余字段处理；如果不影响结果口径，就尽早删除，而不是一路带到末端。
- 对于上游出现但下游不消费的排序字段、中间列，优先直接删除其计算逻辑，而不是仅在结果表末端清理。
...
- 单个 stage / 环节读取行数超过 10 亿，或者输入规模超过 2T:
  - 先检查是否合理，重点看分区裁剪、字段裁剪、重复扫描、上游表粒度和是否需要中间表
- 总数据量不大但 stage 仍然很慢:
...
- `P2`
  - 当单次 scan 量过大、或者长历史分区被反复扫描时，优先考虑构建中间表，把重扫描变成可复用的落地结果
  - 当超大 stage 超过 4 个时，优先考虑把部分 stage 落表 / 中间表化，避免中途失败导致前面已完成的数据无法复用
