...
- 如果读取的数据量不大，但文件数很多，或者平均单文件大小小于 16M，要优先怀疑上游表存在小文件问题；重点检查是否需要合并小文件、调整写入粒度、重建中间表或重新分区。
...
- 读取数据量不大，但文件数很多，或者平均单文件大小小于 16M:
  - 优先怀疑上游表存在小文件问题，重点看文件合并、写入粒度和是否需要重建中间表
- 总数据量不大但 stage 仍然很慢:
...
- `persist / cache`
  - 只在会被重复使用且体量可控时考虑
- 小文件问题
  - 读取量不大但文件数很多、或者平均单文件大小小于 16M 时，优先怀疑小文件
  - 优先考虑合并小文件、调整分区和写入粒度、构建中间表或重建上游表
