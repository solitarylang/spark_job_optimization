...
- 如果读取的数据量不大，但文件数很多，或者平均单文件大小小于 16M，要先标成小文件风险候选，重点检查是否存在上游小文件、合并缺失、写入粒度过细或需要中间表
...
- 每个 stage 的小文件风险候选；如果读取量不大但文件数很多，或者平均单文件大小小于 16M，要明确写出
- 每个 stage 对应的数据文件数、平均文件大小或可推导的小文件证据
- 需要在运行日志分析阶段补齐的证据缺口
